...
export async function deleteMovie(request, response) {
  ...
  try {
    ...
    return response.status(200).json({ message: "Movie deleted successfully.", movie: deletedMovie });
  } catch (error) {
    console.error(`Error deleting movie: ${error.message}`);
    return response.status(500).json({ message: "Internal Server Error. Check console for details" });
  }
}